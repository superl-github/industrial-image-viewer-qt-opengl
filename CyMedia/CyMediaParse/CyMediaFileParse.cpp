#include "CyMediaFileParse.h"

#include <QFile>
#include <QFileInfo>

namespace CyMedia {
    CyMdiaImageParse::CyMdiaImageParse(QObject* parent /*= nullptr*/) 
    : QObject(parent){

    }

    QString CyMdiaImageParse::fileTypeStr(CyMedia::ImageSuffix type) {
        switch (type) {
        case CyMedia::IMAGE_SUFFIX_RAW: return "raw";
        case CyMedia::IMAGE_SUFFIX_BMP: return "bmp";
        case CyMedia::IMAGE_SUFFIX_TIFF: return "tiff";
        }
        return "Undefined";
    }


    CyMedia::ImageSuffix CyMdiaImageParse::getTypeByPath(QString& filepath) {
        QFileInfo fileInfo(filepath);
        QString suffix = fileInfo.suffix().toLower();
        if (suffix == "raw") {
            return IMAGE_SUFFIX_RAW;
        }
        else if (suffix == "bmp") {
            return IMAGE_SUFFIX_BMP;
        }
        else if (suffix == "tiff" || suffix == "tif") {
            return IMAGE_SUFFIX_TIFF;
        }

        return IMAGE_SUFFIX_INVALID;
    }


    int CyMdiaImageParse::openImage(QString filePath, CyMedia::ImageSuffix fileType, CyMedia::ImageShowInfo& info, std::vector<uint8_t>& data) {
        QFile tFile(filePath);
        if (false == tFile.exists()) return 1;
        switch (fileType) {
            case CyMedia::IMAGE_SUFFIX_RAW: {
                if (false == tFile.open(QIODevice::ReadOnly)) {
                    return 1;
                }
                uint32_t fileSize = tFile.size();
                //读取头
                int headSize = sizeof(CyMedia::ImageShowInfo);
                if (fileSize < headSize) return 1;
                auto headCode = tFile.read(headSize);
                if (headCode.size() != headSize) {
                    tFile.close();
                    return 1;
                }
                memcpy(&info, headCode.data(), headSize);
                //验证长度
                if (fileSize != info.length + headSize) {
                    tFile.close();
                    return 3;
                }
                data.resize(info.length);
                auto imgCode = tFile.readAll();
                tFile.close();
                if (imgCode.size() != info.length) {
                    return 1;
                }
                memcpy(data.data(), imgCode.data(), info.length);
                return 1;
            }break;

            case CyMedia::IMAGE_SUFFIX_BMP: {
                return 2;
            }break;

            case CyMedia::IMAGE_SUFFIX_TIFF: {
                return 2;
            }break;

            case CyMedia::IMAGE_SUFFIX_PNG: {
                return 2;
            }break;

            case CyMedia::IMAGE_SUFFIX_JPEG: {
                return 2;
            }break;

        }

        return 2;
    }


    int CyMdiaImageParse::openImage_NotHeaderRaw(QString filePath, int dataOffset, CyMedia::ImageShowInfo& info, std::vector<uint8_t>& data) {
        QFile rawFile(filePath);
        if (false == rawFile.exists()) return 1;
        uint32_t fileSize = rawFile.size();
        if (fileSize < dataOffset + info.length) return 1;
        if (false == rawFile.open(QIODevice::ReadOnly)) return 1;
        rawFile.seek(dataOffset);
        auto imgCode = rawFile.read(info.length);
        rawFile.close();
        if (imgCode.size() != info.length) return 1;
        data.resize(info.length);
        memcpy(data.data(), imgCode.data(), info.length);
        return 0;
    }


    int CyMdiaImageParse::saveImageToFile(QString filePath, const CyMedia::ImageShowInfo& info, const uint8_t* data, bool addRawHeader) {
        auto fileType = getTypeByPath(filePath);
        switch (fileType) {
            case CyMedia::IMAGE_SUFFIX_RAW: {
                QFile tFile(filePath);
                if (false == tFile.open(QIODevice::ReadOnly)) return 1;
                tFile.write((char*)(&info));
                tFile.write((char*)data, info.length);
                tFile.waitForBytesWritten(1000);
                return 0;
            }break;

            case CyMedia::IMAGE_SUFFIX_BMP: {
                return 3;
            }break;

            case CyMedia::IMAGE_SUFFIX_TIFF: {
                return 3;
            }break;

            case CyMedia::IMAGE_SUFFIX_PNG: {
                return 3;
            }break;

            case CyMedia::IMAGE_SUFFIX_JPEG: {
                return 3;
            }break;
        }

        return 3;
    }

};
